...
#define COUNT_SPEED 50       // How quickly the counters increment
#define GREEN_ZONE_MIN .2   // Percentage of total volume required for the green counter to increment
#define YELLOW_ZONE_MIN .5  // Percentage of total volume required for the yellow counter to increment
#define RED_ZONE_MIN .8    // Percentage of total volume required for the red counter to increment
...
//Zone Counters
Adafruit_7segment zoneCounters[3] = {Adafruit_7segment(),Adafruit_7segment(),Adafruit_7segment()};
unsigned int zoneCounterQuantity[3] = {0,0,0};
...
void incrementCounters() {
  ...
  averageOverPeriod = (averageOverPeriod + height)/2;
  Serial.println(averageOverPeriod/N_PIXELS);

  counterCounter++;
  if (counterCounter > COUNT_SPEED) {
    counterCounter = 0;
    int zone = -1;
    if (averageOverPeriod / N_PIXELS >= RED_ZONE_MIN) {
      zone = 2;
    } else if (averageOverPeriod / N_PIXELS >= YELLOW_ZONE_MIN) {
      zone = 1;
    } else if (averageOverPeriod / N_PIXELS >= GREEN_ZONE_MIN) {
      zone = 0;
    }
    averageOverPeriod = height;
    if (zone >= 0) {
